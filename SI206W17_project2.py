...
try:
  jsonfile = open("206project2_caching.json", 'r') # Try to read the data from the file
  cache_contents = twitterFile.read() # If it's there, get it into a string
  CACHE_DICTION = json.loads(cache_contents) # And then load it into a dictionary
  jsonfile.close() # Close the file, we're good, we got the data in a dictionary.
except:
...
def find_urls(string):
	urls = []
	url_group = []
	url_group = re.findall('((http|https)://.+?)\s', string) #pulls a tuple with http/https and full url
...
def get_umsi_data():
	if 'umsi_directory_data' in CACHE_DICTION:
		...
		while page < 12:
			...
			results = umsi_data
			json_pages.append(json.dumps(results))
		CACHE_DICTION['umsi_directory_data'] = json_pages
    	# CACHE_DICTION['umsi_directory_data'] = results   


		# CACHE_DICTION["umsi_directory_data"] = umsi_pages
		jsonFile = open('206project2_caching.json', 'w')
		jsonFile.write(str(CACHE_DICTION))
		jsonFile.close()
...
# only run this on data for the pages we are running on 
umsi_titles = {}
umsi_data = get_umsi_data()
for page in umsi_data:
	...
	for person in people:
		...
		# if person_name not in umsi_titles:
